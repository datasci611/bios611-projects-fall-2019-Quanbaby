...
From the summary statistics and plot we know that most clients came to the shelter were male (~80%). There were also few trans females clients.

### (3) Summary about clients ages


```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
ggplot(client_final, mapping = aes(x = `Client Age at Entry`, fill = `Client Gender`, color = `Client Gender` )) + 
  geom_histogram(position = 'dodge') +  
  theme(legend.text = element_text(size = 8), legend.position = 'bottom') + 
  labs(title = "Histogram of client age at entry (stratified by gender)")
```

From the plot we know most of the clients were ~50 years old, but there was a small peak at ~22 years old. 
...
Most people left to stay with friends for temporary tenure.
...
From the above plot, most people stayed for less than 500 days. To better see the details, we zoom in the plot in the (0, 500) interval.

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
ggplot(client_final %>% filter(`Time Spent (days)`<500), mapping = aes(`Time Spent (days)`)) + 
  geom_density() + 
...
We then want fit a regression model where the response variable is client staying days. The first question is about the explanatory variables. Potential explanotory variables are client age at entry, client gender, client race, client veteran status, client income at entry, client insurance types at entry, clinet noncash services at entry, client disabilities at entry. We first convert the catogorical data to numericals.
...
#### c. Fit the model
We first know that the distribution of staying length is not normal, thus we do a log transformation first and use the 'log(days)' as our response variable. And remove outliers, which are defined as >500 days or =0 day.

We use OLS function in statsmodels.api package in python to fit a regression model. OLS is short for "Ordinary Least Square", which is a method to generate a linear model. The model summary result is shown below:

![](./model.png)

The model R square is 0.732 which is not too bad. From the model fitting results, we can see in general,

1. veterans tended to stay in the shelter shorter than non-veterans,

2. men tended to stay in the shelter longer than women,

3. older people tended to stay in the shelter longer than younger people,

4. black people tended to stay in the shelter longer than white people.

If we use more stringent principles (>500 days or <=2 days) for the outliers, the model R square will be 0.838.

![](./model2.png)

But we still couldn't predict the staying length pretty well since it's really objective. Different people may have different thoughts and preferences. It seems most people would treat the shleter as a temporary place to stay, and they may leave quickly after successfully contacting their friends/families.
...
The reason we give "Income diff" weight $0.0004$ is that the values of income difference are around ~1000. This weight may avoid the case that this variable is the dominant one among the four variables.
